...
export class BoardThreadPostController {
  public constructor(
  ...
  public show = async (ctx: Koa.Context) => {
    ...
    if (post === null || (post.parentId !== thread.id && post.id !== thread.id) || post.board.id !== board.id) {
  ...
  public create = async (ctx: Koa.Context) => {
    ...
    ctx.status = 201;
    ctx.set('Location', `/api/v1/boards/${board.slug}/threads/${thread.id}/posts/${post.id}`);
    ctx.body = { item: this.convertModelToDto(post) };
  };
...
export default BoardThreadPostController;
