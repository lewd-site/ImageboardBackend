...
test('create post', async () => {
  ...
  const response = await request(app?.callback())
    .post('/api/v1/posts')
    .send({ parentId: 1, name: 'Tester', message: 'Test reply 1' });
...
test('get posts', async () => {
  ...
  await request(app?.callback()).post('/api/v1/posts').send({ parentId: 1, name: 'Tester', message: 'Test reply 1' });
  await request(app?.callback()).post('/api/v1/posts').send({ parentId: 1, name: 'Tester', message: 'Test reply 2' });

  // Act
  const response = await request(app?.callback()).get('/api/v1/posts');
...
test('get post', async () => {
  ...
  // Act
  const response = await request(app?.callback()).get('/api/v1/posts/3');
...
test('get missing post', async () => {
  ...
  // Act
  const response = await request(app?.callback()).get('/api/v1/posts/4');
...
test('delete post without auth', async () => {
  ...
  // Act
  const response = await request(app?.callback()).delete('/api/v1/posts/3');
