...
test('get posts', async () => {
  ...
  await request(app.callback()).post('/api/v1/boards').send({ slug: 'a', title: 'Anime' });
  await request(app.callback()).post('/api/v1/boards/a/threads').send({ name: 'Tester', message: 'Test thread 1' });
  await request(app.callback()).post('/api/v1/threads/1/posts').send({ name: 'Tester', message: 'Test reply 1' });
  await request(app.callback()).post('/api/v1/threads/1/posts').send({ name: 'Tester', message: 'Test reply 2' });

  // Act
  const response = await request(app.callback()).get('/api/v1/threads/1/posts');
...
test('get post', async () => {
  ...
  // Act
  const response = await request(app.callback()).get('/api/v1/threads/1/posts/3');
...
test('get missing post', async () => {
  ...
  // Act
  const response = await request(app.callback()).get('/api/v1/threads/1/posts/4');
...
test('delete post', async () => {
  ...
  // Act
  const response = await request(app.callback())
    .delete('/api/v1/threads/1/posts/3')
...
test('delete post without auth', async () => {
  ...
  // Act
  const response = await request(app.callback()).delete('/api/v1/threads/1/posts/3');
...
test('delete missing post', async () => {
  ...
  // Act
  const response = await request(app.callback())
    .delete('/api/v1/threads/1/posts/4')
    .set('Authorization', `Bearer ${config.auth.token}`);
